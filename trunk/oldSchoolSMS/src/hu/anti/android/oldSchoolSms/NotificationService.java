...
public class NotificationService extends IntentService {
	...
	PendingIntent sentPI = PendingIntent.getBroadcast(
		...
		SentStatusReceiver.getIntent(getApplicationContext(), uri), 0);

	// create delivery listener
	PendingIntent deliveredPI = PendingIntent
		.getBroadcast(getApplicationContext(), 0,
			DeliveredStatusReceiver.getIntent(
				getApplicationContext(), uri), 0);
	...
	// execute send
	smsManager.sendMultipartTextMessage(address, null, dividedMessage,
		sentIntents, deliveryIntents);
    }

    public static Uri putNewSmsToDatabase(ContentResolver contentResolver,
	    String address, String body, String type, String status) {
	// Create SMS row
	...
	Uri uri = contentResolver.insert(Uri.parse(Sms.Uris.SMS_URI_BASE),
		values);
	// FIXME reupdate...
	...
	String title = context.getResources().getString(
		R.string.SMS_RECIVED_TITLE);
	String message = context.getResources().getString(
		R.string.SMS_RECIVED_COUNT);
	...
	Cursor cursor = context.getContentResolver().query(
		uri,
		null,
		"(" + Sms.Fields.TYPE + " = " + Sms.Type.MESSAGE_TYPE_INBOX
			+ " AND " + Sms.Fields.READ + " = 0)", null, null);
